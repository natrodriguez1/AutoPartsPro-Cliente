// src/features/cart/components/CheckoutView.tsx
...
export function CheckoutView({ items, onRegresar, onCompletado }: CheckoutViewProps) {
        ...
        <div className="flex items-center space-x-4">
          {[1, 2, 3, 4].map((paso) => (
            <div key={paso} className="flex items-center">
              ...
              {paso < 4 && (
                <div className={`w-16 h-0.5 ml-4 ${paso < pasoActual ? "bg-primary" : "bg-muted"}`} />
              )}
            </div>
          ))}
        </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                      ...
                      <div key={direccion.id} className="flex items-start space-x-3 p-4 border rounded-lg">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            {direccion.principal && <Badge variant="secondary" className="text-xs">Principal</Badge>}
                          </div>
                          <p className="text-sm text-muted-foreground mt-1">{direccion.nombre}</p>
                  ...
                  <div className="flex justify-end">
                    <Button onClick={() => setPasoActual(2)}>Continuar</Button>
                  </div>
                </CardContent>
              </Card>
            </motion.div>
                ...
                <CardContent className="space-y-4">
                  <RadioGroup value={metodoEnvio} onValueChange={setMetodoEnvio}>
                    ...
                    <div className="flex items-start space-x-3 p-4 border rounded-lg">
                      ...
                      <div className="flex-1">
                        <Label htmlFor="pickup" className="font-medium">Recoger en Tienda</Label>
                  ...
                  <div className="flex justify-between">
                    <Button variant="outline" onClick={() => setPasoActual(1)}>Atrás</Button>
                    <Button onClick={() => setPasoActual(3)}>Continuar</Button>
                  </div>
                </CardContent>
              </Card>
            </motion.div>
              ...
              <Card>
                ...
                <CardContent>
                    ...
                    <TabsContent value="tarjeta" className="space-y-4 mt-6">
                      ...
                      <RadioGroup value={tarjetaSeleccionada} onValueChange={setTarjetaSeleccionada}>
                        {tarjetasGuardadas.map((tarjeta) => (
                          <div key={tarjeta.id} className="flex items-center space-x-3 p-4 border rounded-lg">
                            ...
                            {tarjeta.principal && <Badge variant="secondary" className="text-xs">Principal</Badge>}
                          </div>
                        ))}
                      </RadioGroup>
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        ...
                        <div className="flex items-center space-x-2">
                          <Checkbox id="guardar" checked={guardarTarjeta} onCheckedChange={setGuardarTarjeta} />
                          <Label htmlFor="guardar" className="text-sm">Guardar esta tarjeta para futuras compras</Label>
                        </div>
                      </div>
                    </TabsContent>
                  ...
                  <div className="flex justify-between mt-6">
                    <Button variant="outline" onClick={() => setPasoActual(2)}>Atrás</Button>
                    <Button onClick={() => setPasoActual(4)}>Continuar</Button>
                  </div>
                </CardContent>
              </Card>
            </motion.div>
              ...
              <Card>
                ...
                <CardContent className="space-y-6">
                  ...
                  <div>
                    ...
                    {metodoPago === "transferencia" && <p className="text-sm text-muted-foreground">Transferencia Bancaria</p>}
                    {metodoPago === "efectivo" && <p className="text-sm text-muted-foreground">Pago Contraentrega</p>}
                  </div>

                  <div className="flex justify-between">
                    <Button variant="outline" onClick={() => setPasoActual(3)}>Atrás</Button>
                    <Button onClick={procesarPago} disabled={procesando} className="min-w-[120px]">
                      {procesando ? (
                        <div className="flex items-center gap-2">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
                ...
                {items.map((item) => (
                  <div key={item.id} className="flex gap-3">
                    <div className="w-12 h-12 flex-shrink-0">
                      <ImageWithFallback src={item.image} alt={item.name} className="w-full h-full object-cover rounded" />
                    </div>
                    <div className="flex-1">
                      <p className="text-sm font-medium line-clamp-2">{item.name}</p>
                      <p className="text-sm text-muted-foreground">Cantidad: {item.cantidad} × ${item.price}</p>
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  <span>Envío</span>
                  <span>{costoEnvio === 0 ? <span className="text-green-600">GRATIS</span> : `$${costoEnvio.toFixed(2)}`}</span>
                </div>
                <Separator />
